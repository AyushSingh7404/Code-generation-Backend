...
class ConversationBuffer:
    ...
    def get_messages_for_api(self) -> List[Dict]:
        ...
        Caching layers:
        1. Examples (first 2 msgs): Already have cache control
...
async def chat_endpoint(
    ...
    """
    # Print incoming request data from frontend
    file_names = [f.filename for f in files] if files else []
    print(f"Frontend chat request: query='{query}', session_id='{session_id}', model_name='{model_name}', workspace_tree_len={len(workspace_tree) if workspace_tree else 0}, files={file_names}")
